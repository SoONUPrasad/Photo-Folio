...
function ImagesList({ albumId }) {
  ...
  useEffect(() => {
    ...
    const getImages = onSnapshot(query(collection(db,"images"),where("albumId", "==", albumId)),
    (snapshot) => {
      const fetchedImages = snapshot.docs.map((doc) => ({
              id: doc.id,
              ...doc.data(),
        ...
        <div>
          <div className="bg-white px-52">
            ...
            <div className="albumForm">
              <h2 className="text-2xl font-bold text-gray-900">Your Image Here</h2>
              {state ? (
                ""
              ) : (
                <button
            ...
            {state ? <ImageForm albumId={albumId} editImageId={editImageId} /> : ""}
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
                  {images.map((image) => (
                    ...
                    >
                      ...
                      <h3 className="mt-4 font-bold text-gray-700">{image.title}</h3>
                    </div>
            ...
            <Carousel images={images} selectedImageId={selectedImage} onClose={handleCloseCarousel} />
          )}
                </div>
              )}
            </div>
